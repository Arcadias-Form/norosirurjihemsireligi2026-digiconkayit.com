<%@ Page Title="" Language="C#" MasterPageFile="~/tr/tr.master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="ArcForm_Web_v2.tr.Default" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="tr_Content" runat="server">
            ...
            <div class="col-md-12 mt-3">
                ...
                <asp:Panel ID="PnlBankaBilgisi" runat="server" Visible="False">
                    <fieldset class="mt-3">
                        ...
                        <p class="text-center" style="color: #fff">Ödemenizi takip edebilmemiz için, dekontunuzu <a href="mailto:....@....">....@....</a> adresine sipariş numarası içerecek şekilde göndermeyi unutmayınız.</p>
                    </fieldset>
                </asp:Panel>
    ...
    <asp:SqlDataSource runat="server" ID="OleDbTransferTipiListesi" ConnectionString='<%$ ConnectionStrings:ArcForm_Web %>' ProviderName='<%$ ConnectionStrings:ArcForm_Web.ProviderName %>' SelectCommand="SELECT [TransferTipiDilTablosu].* FROM [TransferTipiDilTablosu] INNER JOIN [TransferTipiTablosu] ON [TransferTipiDilTablosu].[TransferTipiID] = [TransferTipiTablosu].[TransferTipiID] WHERE [TransferTipiDilTablosu].[DilID] = 'tr' ORDER BY Sira"></asp:SqlDataSource>

    <asp:SqlDataSource runat="server" ID="OleDbKursListesi" ConnectionString='<%$ ConnectionStrings:ArcForm_Web %>' ProviderName='<%$ ConnectionStrings:ArcForm_Web.ProviderName %>' SelectCommand="SELECT [KursTipiDilTablosu].* FROM [KursTipiDilTablosu] INNER JOIN [KursTipiTablosu] ON [KursTipiDilTablosu].[KursTipiID] = [KursTipiTablosu].[KursTipiID] WHERE [KursTipiDilTablosu].[DilID] = 'tr' ORDER BY Sira"></asp:SqlDataSource>

    <asp:SqlDataSource runat="server" ID="OleDbEtkinlikListesi" ConnectionString='<%$ ConnectionStrings:ArcForm_Web %>' ProviderName='<%$ ConnectionStrings:ArcForm_Web.ProviderName %>' SelectCommand="SELECT [EtkinlikDilTablosu].* FROM [EtkinlikDilTablosu] INNER JOIN [EtkinlikTablosu] ON [EtkinlikTablosu].[EtkinlikID] = [EtkinlikDilTablosu].[EtkinlikID] WHERE [EtkinlikDilTablosu].[DilID] = 'tr' ORDER BY Sira"></asp:SqlDataSource>

    <asp:SqlDataSource runat="server" ID="OleDbOdemeTipiListesi" ConnectionString='<%$ ConnectionStrings:ArcForm_Web %>' ProviderName='<%$ ConnectionStrings:ArcForm_Web.ProviderName %>' SelectCommand="SELECT * FROM [OdemeTipiDilTablosu] WHERE [DilID] = 'tr'"></asp:SqlDataSource>
</asp:Content>
